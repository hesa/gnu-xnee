...
xnee_print_event_info (xnee_data *xd) 
{
  ...
  fprintf (xd->out,"  X11 Event\n\n");
  fprintf (xd->out,"number\tname\n");
  fprintf (xd->out,"======\t====\n");
...
xnee_print_sys_info(xnee_data *xd, FILE *out)
{
  ...
  if (( xd->out_name == NULL ) && (xd->recorder==0))
    return 0;
